...
import { 
  Plus, RefreshCw, Search, Trash2, Salad, 
  FileText, ExternalLink, X, Loader2, CheckCircle, AlertCircle, UploadCloud,
...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`fixed bottom-4 right-4 ${bgColors} text-white px-6 py-3 rounded-lg shadow-xl flex items-center gap-3 animate-in slide-in-from-bottom-5 z-50`}>
      {icon}
  ...
  >
    <div className="flex items-start justify-between mb-4">
      ...
      <div className="flex gap-1">
        ...
        <button
            onClick={(e) => { e.stopPropagation(); onDelete(dieta.id); }}
    ...
    <div>
      <h3 className="font-bold text-gray-800 text-lg mb-1 line-clamp-1" title={dieta.descricao}>
        {dieta.descricao}
      </h3>
      <div className="flex items-center gap-2 text-sm text-gray-500 mt-2">
        <FileText size={14} />
        <span className="truncate max-w-[200px]">{dieta.filename || "Arquivo anexado"}</span>
      </div>
    </div>
...
export default function DietaDashboard() {
  ...
  useEffect(() => { loadDietas(); }, [loadDietas]);
  ...
  useEffect(() => {
    if (!search) {
      ...
      setFilteredDietas(dietas.filter(d => d.descricao?.toLowerCase().includes(lowerSearch)));
    }
  }, [search, dietas]);
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-4 md:p-8 font-sans text-gray-900">
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div>
             <h1 className="text-3xl font-extrabold text-gray-900 flex items-center gap-3">
               <span className="bg-green-100 text-green-600 p-2 rounded-lg"><Salad size={28}/></span>
               Gestão de Dietas
             </h1>
             <p className="text-gray-500 mt-2 ml-1">Organize os planos alimentares e arquivos.</p>
          </div>
        ...
        {loading ? (
           ...
           <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300">
              ...
              <p className="text-gray-500 font-medium">Nenhum plano alimentar encontrado.</p>
           </div>
        ) : (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      {/* --- MODAL DE CRIAÇÃO --- */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && setIsModalOpen(false)}>
           <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg animate-in fade-in zoom-in duration-200 relative">
              <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500 p-2 rounded-full hover:bg-gray-100 transition"><X size={20}/></button>
              
              <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-4">Cadastrar Nova Dieta</h2>
              
              <form onSubmit={handleSubmit} className="space-y-5">
                 <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Descrição do Plano</label>
                    <textarea 
                       rows={3}
                 ...
                 <div>
                    ...
                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-green-50 hover:border-green-300 transition group">
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            <UploadCloud size={32} className="text-gray-400 group-hover:text-green-500 mb-2 transition-colors"/>
                            <p className="text-sm text-gray-500 group-hover:text-green-600 font-medium">
                               {fileNamePreview ? fileNamePreview : "Clique para selecionar o arquivo"}
                            </p>
                        </div>
                 ...
                 >
                    {submitting ? <Loader2 className="animate-spin" /> : "Salvar Dieta"}
                 </button>
              </form>
           </div>
        </div>
      ...
      {viewModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && setViewModal(null)}>
           <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md animate-in fade-in zoom-in duration-200 relative text-center">
              <button onClick={() => setViewModal(null)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500 p-2 rounded-full hover:bg-gray-100 transition"><X size={20}/></button>
              
              <div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4">
                 <FileText size={32} />
              </div>

              <h3 className="text-xl font-bold text-gray-900 mb-2">{viewModal.descricao}</h3>
