...
export default function ExercicioDashboard() {
  ...
  useEffect(() => {
    ...
    if (publicFilters.categoria) {
      filtered = filtered.filter((ex) => ex.Categoria === publicFilters.categoria);
    }
    ...
    try {
        ...
        if (exPublico.videos?.[0]?.url) {
            try {
                ...
                if (!videoResponse.ok) throw new Error("Falha ao baixar vídeo");
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
      <div className="max-w-7xl mx-auto bg-white shadow-2xl rounded-2xl p-6 sm:p-10 border border-gray-100">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl font-semibold hover:bg-blue-700 transition-all shadow-md active:scale-95">
              <Plus size={20} /> Novo
            </button>
            <button onClick={handleOpenPublicLibrary} className="flex items-center gap-2 bg-purple-600 text-white px-4 py-2 rounded-xl font-semibold hover:bg-purple-700 transition-all shadow-md active:scale-95">
              <Globe size={20} /> Biblioteca Pública
            </button>
            <button onClick={loadExercicios} className={`flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2 rounded-xl hover:bg-gray-100 transition active:scale-95 ${loading ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={loading}>
              {loading ? <Loader2 size={20} className="animate-spin" /> : <RefreshCw size={20} />} {loading ? "Carregando..." : "Atualizar"}
            </button>
            <button onClick={() => setIsFilterOpen(!isFilterOpen)} className="md:hidden flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2 rounded-xl hover:bg-gray-100 transition active:scale-95">
        ...
        <div className="hidden md:block">
           <FilterBar currentFilters={filters} setFilterState={setFilters} catOptions={categorias} grpOptions={grupos} placeholder="Buscar exercício por nome..." />
        </div>
        {isFilterOpen && (
          <div className="md:hidden bg-gray-50 p-4 rounded-xl mb-6 border border-gray-200 shadow-inner">
             <FilterBar currentFilters={filters} setFilterState={setFilters} catOptions={categorias} grpOptions={grupos} placeholder="Buscar exercício..." />
          </div>
        )}
        ...
        {loading ? (
          ...
          <p className="text-gray-500 text-center py-10 text-lg">Nenhum exercício encontrado.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
             {filteredExercicios.map((ex) => (
                <div key={ex.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-md hover:shadow-xl transition flex flex-col justify-between">
                    <div>
                        <div className="flex justify-between items-start mb-2">
                           <h3 className="font-bold text-lg text-gray-800 break-words max-w-[80%]">{ex.nome}</h3>
                           <button onClick={(e) => { e.stopPropagation(); setEditModal(ex); }} className="text-gray-400 hover:text-blue-500 p-1 rounded-full hover:bg-gray-100"><Edit3 size={18} /></button>
                        </div>
                        <p className="text-gray-500 text-sm mb-1 flex items-center gap-1"><List size={14} className="text-gray-400" /> {ex.Grupo_Muscular}</p>
                        <p className="text-gray-500 text-sm mb-3 flex items-center gap-1"><Dumbbell size={14} className="text-gray-400" /> {ex.Aparelho || "Livre"}</p>
                    </div>
                    <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                        {ex.videos?.[0]?.url ? (
                          <span className="text-xs text-green-600 font-medium flex items-center gap-1"><Video size={14} /> Vídeo Anexado</span>
                        ) : (
                          <span className="text-xs text-orange-600 font-medium flex items-center gap-1"><Video size={14} /> Sem Vídeo</span>
                        )}
                        <button onClick={(e) => { e.stopPropagation(); handleDelete(ex.id); }} className="text-red-600 hover:bg-red-50 p-1 rounded-md text-sm"><Trash2 size={16} /></button>
                    </div>
                </div>
             ))}
          </div>
      ...
      {showModal && (
        <Modal title="Adicionar Novo Exercício" onClose={() => setShowModal(false)} onSubmit={handleSubmit} submitting={submitting}>
          <div className="space-y-4">
            <input type="text" placeholder="Nome" value={form.nome} onChange={(e) => setForm({...form, nome: e.target.value})} required className="w-full border p-2 rounded-xl" />
            <input type="text" placeholder="Categoria" value={form.Categoria} onChange={(e) => setForm({...form, Categoria: e.target.value})} required className="w-full border p-2 rounded-xl" />
      ...
      {editModal && (
        <Modal title={`Editando: ${editModal.nome}`} onClose={() => setEditModal(null)} onSubmit={handleUpdate} submitting={submitting} size="max-w-5xl">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1.5fr] gap-6 items-start">
            <div className="md:sticky md:top-0"><VideoPreview editModal={editModal} setEditModal={setEditModal} /></div>
            <div className="space-y-4">
              <input type="text" placeholder="Nome" value={editModal.nome} onChange={(e) => setEditModal({...editModal, nome: e.target.value})} className="w-full border p-2 rounded-xl" />
              <div className="grid grid-cols-2 gap-4">
                  <input type="text" placeholder="Categoria" value={editModal.Categoria} onChange={(e) => setEditModal({...editModal, Categoria: e.target.value})} className="w-full border p-2 rounded-xl" />
                  <input type="text" placeholder="Grupo" value={editModal.Grupo_Muscular} onChange={(e) => setEditModal({...editModal, Grupo_Muscular: e.target.value})} className="w-full border p-2 rounded-xl" />
              </div>
              <input type="text" placeholder="Aparelho" value={editModal.Aparelho || ""} onChange={(e) => setEditModal({...editModal, Aparelho: e.target.value})} className="w-full border p-2 rounded-xl" />
              <textarea rows={6} placeholder="Descrição" value={editModal.Descricao} onChange={(e) => setEditModal({...editModal, Descricao: e.target.value})} className="w-full border p-2 rounded-xl resize-none" />
            </div>
          </div>
        </Modal>
      )}
      ...
      {showPublicModal && (
        ...
        >
          <div className="mb-6 bg-gray-50 p-4 rounded-xl border border-gray-200">
            ...
            <FilterBar currentFilters={publicFilters} setFilterState={setPublicFilters} catOptions={publicCategorias} grpOptions={publicGrupos} placeholder="Pesquisar na biblioteca pública..." />
          </div>

          {loadingPublic ? (
            <div className="flex justify-center items-center py-20">
               <Loader2 size={32} className="animate-spin mr-3 text-purple-600" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr] gap-6">
                ...
                <div className="flex flex-col h-full">
                    <div className="space-y-4 flex-1">
                        <div>
                            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Dados Técnicos</span>
                        ...
                        <div>
                            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Descrição / Execução</span>
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-100">
                        ...
                        >
                            {importingId === viewPublicModal.id ? <Loader2 size={24} className="animate-spin"/> : <Download size={24} />}
                            {importingId === viewPublicModal.id ? "Importando..." : "Importar Agora"}
                        </button>
                    </div>
                </div>
            </div>
...
const VideoPreview = ({ editModal, setEditModal }) => {
    const videoSrc = editModal.newFile ? URL.createObjectURL(editModal.newFile) : editModal.videos?.[0]?.url;
    return (
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 h-full flex flex-col">
            ...
            <div className="w-full flex justify-center mb-4 flex-grow bg-black/5 rounded-xl min-h-[300px] items-center">
                {videoSrc ? (
                    ...
                    <div className="flex flex-col items-center gap-2 text-gray-400"><Video size={32} /> Nenhum vídeo</div>
                )}
            </div>
            <label className="cursor-pointer w-full text-center px-4 py-3 rounded-xl text-sm font-medium bg-white border border-gray-200 text-gray-700 hover:border-blue-500 transition shadow-sm">
                ...
                <input type="file" accept="video/*" onChange={(e) => { if(e.target.files[0]) setEditModal({ ...editModal, newFile: e.target.files[0] }); }} className="hidden" />
            </label>
        </div>
    );
};